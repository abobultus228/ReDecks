...
function selFromGroup(g: InventoryGroup): Sel {
  ...
  return {
    templateId: g.card.id,
    coverMid: mid,
    coverHigh: high,
    instanceIds: (g.cards ?? []).map((c) => c.id),
    count: 0,
  };
}
...
function CardCell({
  mid, high, count, available, onDec, onInc,
}: {
  mid: string; high: string; count: number; available: number;
  ...
  return (
    <div style={st.cell}>
      <div style={st.cellImg} onClick={() => zoom(high || mid)}>
        {vid ? (
...
function FormGrid({ sels, onChange }: { sels: Record<number, Sel>; onChange: (id: number, delta: number) => void }) {
    ...
    <div style={st.grid}>
      {items.map((sel) => (
        <CardCell
          ...
          onDec={() => onChange(sel.templateId, -1)}
          onInc={() => onChange(sel.templateId, +1)}
        />
      ))}
    </div>
  );
...
export default function ExchangeBuilder({
  ...
  const [selCreator, setSelCreator] = useState<Record<number, Sel>>({});
  const [selPartner, setSelPartner] = useState<Record<number, Sel>>({});
  ...
  const loadInv = useCallback(
    async (side: Side) => {
      ...
      try {
        ...
        const { results, hasMore } = await getInventoryCards(token, sideUserId(side), next);
        setInv((prev) => {
          const c = prev[side];
        ...
        setInv((prev) => ({ ...prev, [side]: { ...prev[side], loading: false, error: msg } }));
      } finally {
        loadingRef.current[side] = false;
      }
    },
    [inv, token, partner.id, ourId]
  );
  ...
  const adjust = (side: Side, tmplId: number, make: () => Sel, delta: number) => {
    const setSel = side === 'creator' ? setSelCreator : setSelPartner;
    setSel((prev) => {
      const base = prev[tmplId] ?? make();
      const count = Math.max(0, Math.min(base.count + delta, base.instanceIds.length));
      const next = { ...prev };
      if (count <= 0) delete next[tmplId];
      else next[tmplId] = { ...base, count };
      return next;
    });
  };
  ...
  return (
    <CardZoomProvider>
      <div style={st.scroll} ref={scrollRef} onScroll={onScroll}>
        ...
        <FormGrid sels={selCreator} onChange={(id, d) => adjust('creator', id, () => selCreator[id], d)} />
        ...
        <FormGrid sels={selPartner} onChange={(id, d) => adjust('partner', id, () => selPartner[id], d)} />
        ...
        <div style={st.filters}>
          <button style={st.filterStub} disabled>Фильтр</button>
          <button style={st.filterStub} disabled>Фильтр</button>
        </div>

        {/* инвентарь активной стороны */}
        <div style={st.grid}>
          {activeInv.groups.map((g) => {
            const sel = activeSel[g.card.id];
            const count = sel?.count ?? 0;
            const available = (g.cards ?? []).length;
            const mid = resolveMediaUrl(g.card.cover?.mid || g.card.cover?.high || '');
            const high = resolveMediaUrl(g.card.cover?.high || g.card.cover?.mid || '');
            return (
              <CardCell
                ...
                count={count}
                available={available}
                onDec={() => adjust(active, g.card.id, () => selFromGroup(g), -1)}
                onInc={() => adjust(active, g.card.id, () => selFromGroup(g), +1)}
              />
            );
          })}
        </div>
